...
export function PricingSection() {
  return (
    <section id="pricing" className="w-full py-16 sm:py-24 lg:py-32 px-4 sm:px-6 lg:px-8 bg-base border-t border-slate-800">
      <div className="max-w-6xl mx-auto">
          ...
          {plans.map((plan, index) => (
            <div
              ...
              className={`rounded-2xl ${
                plan.featured
                  ? 'border-2 border-accent bg-surface shadow-[0_0_40px_-8px_rgba(163,230,53,0.25)] scale-100 md:scale-105'
                  : 'border border-slate-800 bg-surface hover:border-slate-700'
              }`}
              style={{ transition: 'border-color 300ms' }}
            >
              {plan.featured && (
                <div className="px-4 sm:px-6 py-2 bg-accent text-black text-xs sm:text-sm font-semibold text-center rounded-t-2xl">
                  Most Popular
                </div>
              )}
              ...
              <div className="p-6 sm:p-8 flex flex-col h-full">
                ...
                <div className="mb-6">
                  <div className="flex items-baseline gap-1">
                    ...
                    {plan.price !== 'Custom' && (
                      <span className="text-sm sm:text-base text-slate-500">/project</span>
                    )}
                  </div>
                </div>

                <button
                  className={`w-full py-3 sm:py-3.5 rounded-lg font-semibold mb-8 text-sm sm:text-base ${
                    plan.featured
                      ? 'bg-accent text-black hover:bg-accent-dark'
                      : 'border border-slate-700 text-slate-300 hover:border-accent/50 hover:text-accent'
                  }`}
        ...
        <div className="text-center mt-12 sm:mt-16">
          <p className="text-slate-500 text-sm sm:text-base">
            ...
            <span className="font-semibold text-slate-300">Schedule a free 30-min consultation</span>{' '}
